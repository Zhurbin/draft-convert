...
function genFragment(node, inlineStyle, lastList, inBlock, blockTags, depth, processCustomInlineStyles, checkEntityNode, checkEntityText, checkBlockType, inEntity) {
  ...
  // Block Tags
  var blockType = checkBlockType(nodeName, node, lastList, inBlock);
  if (!inBlock && (blockTags.indexOf(nodeName) !== -1 || blockType)) {
    chunk = getBlockDividerChunk(blockType || getBlockTypeForTag(nodeName, lastList), depth);
    ...
    inBlock = blockType;
    newBlock = true;
    chunk = getSoftNewlineChunk(blockType, depth);
  }
  ...
  while (child) {
    ...
    if (!entityId) {
      ...
      if (child.src) {
        entityId = _draftJs.Entity.create('IMAGE', 'IMUTABLE', { src: child.src });
      }
    }
